...
export class RoomStage {
  ...
  /**
   * The whole room is a single pre-rendered image — walls, desk, monitor,
   * bed, bookshelf, everything lives in the art file, stretched to fill the
   * ROOM_PX_W x ROOM_PX_H footprint the tile-drawn room used to occupy so
   * every other coordinate (MONITOR_SCREEN, DESK, VISITOR_SLOTS) stays
   * meaningful. Avatars render on `sortLayer`, on top of it.
   */
